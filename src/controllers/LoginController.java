...
public class LoginController implements Initializable {
    ...
    private Boolean loginValidation(String username, String password) throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        try (Connection conn = Database.initDatabase()) {
            ...
            if (!employee.next()) {
                ...
                byte[] salt = employee.getBytes("salt");
                byte[] password2 = employee.getBytes("password");
                Boolean auth = encryption.authenticate(password, password2, salt);
                if (auth) {
     ...
     */
    @FXML
    protected void Login(ActionEvent event) throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
